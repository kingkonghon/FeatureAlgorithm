...
def deleteObsoleteDataFromDB(code, new_data_start_date, target_date_field, target_code_field, target_table_name, db_config):
    # delete data in target table that later (become obsolete)
    sql_statement = "delete from `%s` where `%s` >= '%s' and `%s` = '%s'" % (target_table_name, target_date_field, new_data_start_date, target_code_field, code)
    con = pymysql.connect(**db_config)
    with con.cursor() as tmp_cur:
        tmp_num = tmp_cur.execute(sql_statement)
        print('delete obsolete record num:', tmp_num)
    con.commit()
    con.close()
...
def checkIfIncre(db_config, source_table_name, target_table_name, date_field, lags, source_condition, align_flag=True):
    ...
    if tmp_data.loc[0, 'num'] == 0:  # target table not exist
        ...
        lags_needed = max([max(lags)*3, 252])  # 计算增量所需要的数据条数（包括用于算MA等指标，MACD, BBRANDS； 考虑到EMA的准确性，用一个比较大的lag）
        tmp_state = "select date from `TRADE_CALENDAR`"
        trade_calendar = readDB(tmp_state, db_config)['date']
        # 计算开始读数据的时间
        try:
            # start_fetch_date = datetime.strptime(last_record_date, '%Y-%m-%d') - timedelta(
            #     days=lags_needed)
            start_fetch_date = trade_calendar[trade_calendar < last_record_date]
            start_fetch_date = start_fetch_date.iloc[-lags_needed]
        except TypeError:
            # start_fetch_date = last_record_date - timedelta(days=lags_needed)
            start_fetch_date = trade_calendar[trade_calendar < datetime.strftime(last_record_date, '%Y-%m-%d')]
            start_fetch_date = start_fetch_date.iloc[-lags_needed]
        ...
        if tmp_data.loc[0, 'timestamp_target'] < tmp_data.loc[0, 'timestamp_source']: # target last record date ealier than source, run incre
            is_full = 0
        elif (tmp_data.loc[0, 'timestamp_target'] == tmp_data.loc[0, 'timestamp_source']) and align_flag:
                # align_flag and (tmp_data.loc[0, 'num_target'] != tmp_data.loc[0, 'num_source']):  # 最新日期一样，但数据条数不一样，重跑全量
            # is_full = 1
            # 周末可能更新了更多数据，比较time_stamp
            try:
                tmp_state = "select max(time_stamp) from %s" %source_table_name
                tmp_source_last_update_time = readDB(tmp_state, db_config)
                tmp_source_last_update_time = tmp_source_last_update_time.iloc[0, 0]

                tmp_state = "select max(time_stamp) from %s" % target_table_name
                tmp_target_last_update_time = readDB(tmp_state, db_config)
                tmp_target_last_update_time = tmp_target_last_update_time.iloc[0, 0]

                if tmp_source_last_update_time > tmp_target_last_update_time: # 有更新，删除当天数据重算
                    con = pymysql.connect(**db_config)
                    with con.cursor() as cursor:
                        tmp_state = "delete from %s where %s = '%s'" % (target_table_name, date_field, tmp_data.loc[0, 'timestamp_target'])
                        cursor.execute(tmp_state)

                    is_full = 2 # 重跑删除的那天
                else:
                    is_full = -1 # 不用跑
            except:
                is_full = -1 # error, ignore it
        else:
            is_full = -1 # 不需要跑
