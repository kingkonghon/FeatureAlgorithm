...
from SelectedDBConfigV2 import ConfigQuant, stockQuoteConf, allMergeDataConf, areaConf, allMergeDataConf2
...
def screenUnwantedStocks(stock_data, st_config, start_date, end_date):
...
class mergeDataClass:
    ...
    def run(self):
        ...
        if (tmp_dtypes.size > 3) and (self.table_name != 'STOCK_INDUSTRY') and (self.table_name != 'DERI_MARKET_INDEX'):
            print(self.table_name, 'origin:')
            print(sql_statement)
            print(data.dtypes)
...
def loadData(start_date, end_date):
    ...
    # ====== screen data ======
    process_basic_data.data = screenUnwantedStocks(process_basic_data.data, STConfig, start_date, end_date)
