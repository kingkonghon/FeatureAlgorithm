...
class BacktestingClass:
    def __init__(self, **kwargs):
        ...
        self.order_file_path = kwargs.get('order_file_path')
    ...
    def reinit(self):
        ...
        # self.current_season = self.start_season
        self.getPredictionFromFile() # reload the first prediction file

    def getPredictionFromFile(self):
        if os.path.exists(self.order_file_path):
            self.prediction = pd.read_csv(self.order_file_path, dtype={'code': str}, index_col=None, header=0)
    ...
    def calTodayReturn(self):
        tot_weights = np.sum([x['weight'] for x in self.current_position.values()])
        if tot_weights > 1:
            print('holding weight > 1')
            raise ValueError
    ...
    def getCumRets(self):
        cum_rets = np.cumprod(1 + self.rets) - 1
        cum_rets = np.hstack([0, cum_rets])
        return cum_rets
    ...
    def getAnalysis(self, output_file_name):
        ...
        print('average holding trade day num:', self.tot_day_num / self.tot_sell_times)
        ...
        # old_strategy = pd.read_csv(r'D:\model_results\top_bottom\old_strategy_hs300_17.csv')  # load cum rets of old strategy
        # old_strategy = old_strategy.loc[(old_strategy['date'] >= self.start_date) & (old_strategy['date'] <= self.end_date)]
        # old_cum_rets = old_strategy['cum_rets'].values
        # plt.figure(1)
        # plt.plot(dt_trade_dates, self.cum_rets)
        # plt.plot(dt_trade_dates, old_cum_rets)
        # plt.plot(dt_trade_dates, index_cum_rets)
        # plt.legend(['rl_stategy', 'old_strategy', 'index'])
        ...
        while self.today_idx < self.tradeday_num:
            ...
            if len(sell_list) > 0:
                for tmp_code in sell_list:
                    ...
                    self.tot_day_num += self.current_position[tmp_code]['holding_length']
                    self.tot_sell_times += 1

                    del self.current_position[tmp_code]  # remove record from holding list, because closed at open price, profit already recorded
