...
class BacktestingClass:
    def __init__(self, **kwargs):
        ...
        self.timing_file_path = kwargs.get('timing_file_path')
    ...
    def init(self, method):
        ...
        self.timing = pd.read_csv(self.timing_file_path)
        self.timing = self.timing.set_index('date')
    ...
    def run(self):
        while self.today_idx < self.tradeday_num:
            ...
            if self.timing.loc[self.getYesterday(), 'predict'] == 1:  # predict tomorrow
                ...
                for tmp_code in tradable_position_codes:
                    if self.current_position[tmp_code]['holding_length'] >= self.current_position[tmp_code]['max_holding_period']:
                        ...
                        tmp_stock_to_close.append(tmp_code)
                tradable_position_codes = [x for x in tradable_position_codes if x not in tmp_stock_to_close]  # remove from the buffer
            else:
                for tmp_code in tradable_position_codes:
                    # close all holding positions if timing signal stop trading
                    del self.current_position[tmp_code]
                tradable_position_codes = []
