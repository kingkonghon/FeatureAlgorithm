...
stockQuoteFileName ={
    ...
    'hs300_weights': 'LZ_CN_STKA_INDEX_HS300WEIGHT.h5'
}
...
class BacktestingClass:
    def __init__(self, **kwargs):
        ...
        self.current_year = kwargs.get('start_year')
        self.current_season = kwargs.get('start_season')
        self.end_year = kwargs.get('end_year')
        self.end_season = kwargs.get('end_season')
        self.season_in_year = kwargs.get('season_in_year')
        self.strategy = kwargs.get('strategy')
        self.prediction_folder_path = kwargs.get('prediction_folder_path')
        self.prd_pre_file_path = kwargs.get('prd_pre_file_path')
        self.prd_precision_threshold = kwargs.get('prd_precision_threshold')
        self.stock_pool_file_path = kwargs.get('stock_pool_file_path')
        ...
        self.checkpoint_period = kwargs.get('checkpoint_period')
        self.prediction = []
        self.predict_precision = []
        self.current_predicition_lastdate = ''
        self.stock_price = []
        self.stock_high = []
        self.stock_close = []
        self.trade_dates = []
        ...
        self.stock_holdings = []
        self.stock_pool = []
        self.current_prd_precision = []

    def init(self, method):
        # get prediction
        self.getPredictionFromFile()
        self.getPredictPrecision()
        self.getStockPool()
        ...
        if method == 'DB':
            ...
            self.hs300_weights = stock_quote['hs300_weights']
    ...
    def getPredictionFromFile(self):
        tmp_full_path = r'%s\stockscore_%ds%d.csv' %(self.prediction_folder_path, self.current_year, self.current_season)
        if os.path.exists(tmp_full_path):
            self.prediction = pd.read_csv(tmp_full_path, dtype={'code': str}, index_col=None, header=0)

            self.current_predicition_lastdate = self.prediction['date'].max()
    ...
    def getStockPool(self):
        self.stock_pool = pd.read_csv(self.stock_pool_file_path, header=None)
        self.stock_pool.loc[:, 'code'] = self.stock_pool[0].apply(lambda x: x[:-3])
        self.stock_pool = self.stock_pool['code'].values

    def getPredictPrecision(self):
        self.predict_precision = pd.read_csv(self.prd_pre_file_path, index_col=None)
        self.predict_precision = self.predict_precision.loc[self.predict_precision['Unnamed: 0'].apply(lambda x: 'precision' in x)]  # drop recall, leave precision
        self.predict_precision = self.predict_precision.rename({'Unnamed: 0': 'category'}, axis=1)

        # get latest precision
        tmp_year_season = '%dS%d' % (self.current_year, self.current_season)
        self.current_prd_precision = self.predict_precision.loc[
            self.predict_precision['category'].apply(lambda x: tmp_year_season in x), ['category', self.prd_precision_threshold]]

        tmp_prd_len = [2, 5, 10, 20]
        tmp_ori_category = list(map(lambda x: tmp_year_season + '_%dDprecision_1' % x, tmp_prd_len))
        tmp_new_category = list(map(lambda x: 'proba_1_%dD' % x, tmp_prd_len))
        tmp_pair = dict(zip(tmp_ori_category, tmp_new_category))
        self.current_prd_precision.loc[:, 'category'] = self.current_prd_precision['category'].replace(tmp_pair)   # change category names
        self.current_prd_precision.loc[:, self.prd_precision_threshold] = \
            self.current_prd_precision[self.prd_precision_threshold] / self.current_prd_precision[self.prd_precision_threshold].sum()  # convert precision into weights
        self.current_prd_precision = self.current_prd_precision.set_index('category')

    def checkIfReloadPredicition(self):
        today = self.getYesterday()
        if today == self.current_predicition_lastdate:  # move to the nex predicition file
            self.current_season += 1
            if self.current_season == self.season_in_year + 1:  # move to next year
                self.current_season = 1
                self.current_year += 1
            self.getPredictionFromFile() # reload predicition
            self.getPredictPrecision() # update the latest predicting precision
    ...
    def getTodayHS300Contituents(self):
        return self.codes[self.hs300_weights[self.today_idx] > 0]
    ...
    def getCumRets(self):
        cum_ret = np.cumprod(1 + self.rets) - 1
        cum_ret = np.hstack([0, cum_ret])
        return cum_ret
    ...
    def getAnalysis(self, output_file_name):
        ...
        tmp_analysis['std'] = np.std(self.rets, ddof=1) * np.sqrt(252)
        ...
        while self.today_idx < self.tradeday_num:
            ...
            for tmp_code in tradable_position_codes:
                if self.current_position[tmp_code]['holding_length'] >= self.current_position[tmp_code]['max_holding_period']:
                    ...
                    tmp_stock_to_closed.append(tmp_code)
            tradable_position_codes = [x for x in tradable_position_codes if x not in tmp_stock_to_closed]  # remove from the buffer
            ...
            if len(sell_list) > 0:
                for tmp_code in sell_list:
                    if tmp_code in tradable_position_codes:
                        tradable_position_codes.remove(tmp_code)
                        del self.current_position[
                            tmp_code]  # remove record from holding list, because closed at open price, profit already recorded
            ...
            if len(tradable_position_codes) > 0:  # not including stocks bought today
                ...
                for tmp_code in tradable_position_codes:
                    ...
                    if highest_ret >= tmp_code_details['tp']:
                        ...
                        tmp_stock_to_closed.append(tmp_code)
                tradable_position_codes = [x for x in tradable_position_codes if x not in tmp_stock_to_closed]
            ...
            self.checkIfReloadPredicition()  # check if need to move onto the nex prediction file
