...
def pivotStockIndustry(tot_data, sw_industry):
    industy = tot_data['industry']
    for tmp_ind in sw_industry:
        tot_data.loc[:, tmp_ind] = industy.apply(lambda x: 1 if x==tmp_ind else 0)

    tot_data = tot_data.drop('industry', axis=1)
