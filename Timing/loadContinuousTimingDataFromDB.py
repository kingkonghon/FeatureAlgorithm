...
from TimingContinuousDBConfig import ConfigQuant, HS300QuoteConf, allMergeDataConf
...
def loadHS300Date(sql_config, hs300_table_config, start_date, end_date):
    ...
    tmp_fields = hs300_table_config['date_field'] + ',' + ','.join(hs300_table_config['fields'])

    sql_statement = "select %s from %s where %s between '%s' and '%s'" % (tmp_fields,
...
class mergeDataClass:
    ...
    def run(self):
        ...
        conn_sql = sql_engine.connect()
        data = pd.read_sql(sql_statement, conn_sql)
